...
app.get(
  ...
  (req, res) => {
    ...
    // Only check token and redirect for login page
    if (page === "/login") {
      const token =
...
// Dashboard route - always serve React app
// The React app will handle authentication via token in localStorage
app.get("/dashboard", (req, res) => {
  // Serve the React app's index.html for client-side routing
  const distIndexPath = path.join(__dirname, "../../dist/index.html");
  const reactIndexPath = path.join(__dirname, "../../index.html");
  
  // Check if dist/index.html exists (React app is built)
  if (fs.existsSync(distIndexPath)) {
    res.sendFile(distIndexPath);
  } else if (fs.existsSync(reactIndexPath)) {
    // In development, serve the React app's source index.html
    res.sendFile(reactIndexPath);
  } else {
    // Fallback: serve the public index.html (landing page)
    res.sendFile(path.join(__dirname, "../../public/index.html"));
  }
});

// Root route handler - serve React app or landing page
// The React app will handle authentication and routing client-side
app.get("/", (req, res) => {
  // Check if user has token in query (from login redirect)
  const token = req.query.token;
  
  if (token) {
    // User is being redirected with token, serve React app
    const distIndexPath = path.join(__dirname, "../../dist/index.html");
    const reactIndexPath = path.join(__dirname, "../../index.html");
...
app.get("*", (req, res, next) => {
  ...
  if (fs.existsSync(distIndexPath)) {
    ...
    // If React app isn't built, serve 404
    next();
  }
});
...
app.get("/api/user/dashboard", authenticateToken, async (req, res) => {
  try {
    ...
    const portfolioValue = portfolios.reduce((sum, portfolio) => {
      return sum + Number(portfolio.totalValue || 0);
    }, 0);
    ...
    const response = {
      ...
      data: {
        user: {
          ...
          investmentPlan: user.investmentPlan || "moderate",
        },
        portfolioValue,
...
const startServer = async () => {
  try {
    ...
    // Only configure OAuth if enabled
