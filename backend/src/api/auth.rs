...
pub async fn register(
    ...
    match auth_service.register(req.email.clone(), req.password.clone(), req.phone_number.clone()).await {
        Ok(token) => Ok(HttpResponse::Ok().json(AuthResponse { token })),
        Err(Error::Database(e)) if e.as_database_error()
            .and_then(|e| e.code())
...
pub async fn login(
    ...
    let token = service.login(credentials.email.clone(), credentials.password.clone()).await?;
